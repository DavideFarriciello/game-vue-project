<template>
  <div>
    <h2 class="text-5xl flex justify-center mt-10 font-bold text-gradient-from-fucsia hover:-translate-y-1 transition duration-300 ease-in-out">Favorites</h2>
    <div v-if="favoritesStore.favorites.length > 0" class="flex flex-wrap justify-center">
      <div v-for="item in favoritesStore.favorites" :key="item.id" >
        <div
          class="flex flex-row space-x-4 bg-white-game mt-8 pt-2 p-4 ml-8 rounded-lg shadow-2xl hover-shadow-red transition-all duration-300 ease-in-out hover:scale-105 w-[96%] h-[200px] ">
    ...
    <div v-else>
      <h2 class="text-5xl flex justify-center mt-8 pb-2 text-gradient-from-fucsia hover:-translate-y-1 transition duration-300 ease-in-out">Any item is being added to the favorites</h2>
    </div>
  </div>
</template>
